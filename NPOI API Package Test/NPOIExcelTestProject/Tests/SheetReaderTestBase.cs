...
namespace NPOIExcelTestProject.Tests
{
    ...
    /// 工作表读取测试基类
    /// </summary>
    public abstract class SheetReaderTestBase
    {
        private readonly TestSheetReaderFixtureBase testSheetReaderFixtureBase;
        private readonly ITestOutputHelper outputHelper;

        public SheetReaderTestBase(TestSheetReaderFixtureBase testSheetReaderFixtureBase, ITestOutputHelper outputHelper)
        {
        ...
        /// 测试当首行索引值小于0时抛出异常
        /// </summary>
        public virtual void Test_Read_WhenFirstRowIndexIsNegative()
        {
        ...
        public virtual void Test_Read_WhenColumnConfigsIsNotEmpty_WithHasBlankRowsAndCols(bool hasHeader, IEnumerable<ColumnConfigAttribute> columnConfigs)
        {
            ...
            //实际table数据(包含两个空行)
            int firstRowIndex = hasHeader ? 1 : 2;
            var actualTable = sheetReader.Read(6 + 2, firstRowIndex: firstRowIndex, hasHeader: hasHeader, columnConfigs: columnConfigs);
            //预计table数据
            var expectedTable = CreateExpectedDataTableWithColumnConfigs();
            //校验列数
            Assert.Equal(expectedTable.Columns.Count, actualTable.Columns.Count);
